...
<main class="max-w-7xl mx-auto m-4 p-4" x-data="{ deleteUser: [], deleteModalOpen: false }">
    ...
    <div class="w-full flex space-x-4 items-center">
        ...
        <div>
            ...
            <p class="text-md text-gray-500">Employee Management Portal</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-6 mt-6 gap-10">
                    ...
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="border overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <?php foreach ($data->get('employees') as $id => $employee): ?>
                                    <tr class="bg-white">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"> <?= $employee['position'] ?> </span>
                                        </td>
                                        <td class="px-6 py-4 space-x-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="<?= $data->url("employees/$id/edit") ?>"
                                               class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            <?php if ($data->get('mode') !== 'edit'): ?>
                                                <button
                                                        @click="() => { deleteUser = { name: '<?= $employee['name'] ?>', id: '<?= $id ?>' }; deleteModalOpen = true; }"
        ...
        <div class="col-span-2 space-y-4">
            <h1 class="text-lg font-bold">
                <?= $data->get('mode') === 'edit' ? 'Edit employee' : 'Create employee' ?>
            </h1>
                ...
                <div class="rounded-md bg-blue-50 p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3 flex-1 md:flex md:justify-between">
                            <p class="text-sm text-blue-700">You are currently editing employee
                                "<?= $data->get('edit')['name'] ?>".</p>
                            <p class="mt-3 text-sm md:mt-0 md:ml-6">
                                <a href="<?= $data->url("employees") ?>"
            ...
            <div class="w-full rounded border p-4">
                <form action="<?= $data->get('mode') === 'list' ? $data->url("employees/submit") : $data->url("employees/{$data->get('edit_id')}/edit") ?>"
                    ...
                    <?= $data->component('input', [
                        ...
                        'type' => 'text',
                        'required' => true,
                        'value' => $data->get('mode') === 'edit' ? $data->get('edit')['position'] : ''
                    ]) ?>
                    ...
                    <?= $data->component('input', [
                        ...
                        'type' => 'number',
                        'required' => true,
                        'value' => $data->get('mode') === 'edit' ? $data->get('edit')['age'] : ''
                    ]) ?>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="deleteModalOpen" x-transition:enter="ease-out duration-300"
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to delete the employee "<span x-text="deleteUser.name"></span>"
                    ...
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <form
                                method="post"
                                :action="'<?= $data->url('employees/') ?>' + deleteUser.id + '/delete' ">
